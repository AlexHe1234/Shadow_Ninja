...
from __init__ import foundation_model  # 引入全局对象 foundation_model
import re
...
INAPPROPRIATE_KEYWORDS = [
    "discriminatory", "obscene", "bullying", "threats",
...
def classify_content_with_llm(content: str) -> str:
    ...
    """
    system_msg = (
        "Please classify the following comment based on the content policy "
        "into one of the categories: C0 (Non-inappropriate content), "
        "C1 (Other inappropriate content), C2 (Inappropriate content, such as discriminatory remarks, obscene content, etc.), "
        "C3 (Harassment or bullying), or C4 (Hate speech or threats)."
    )
...
def is_inappropriate(content: str) -> bool:
    ...
    # 如果分类结果不是 C0，则认为是不适当的内容
    if classification != "C0":
        print(f"Inappropriate content detected: {classification}")
        return True
...
def moderate_comments(courses: List[Dict]) -> (List[Dict], List[Dict]):
    """过滤掉不良评论并保留被删除的评论记录"""
    filtered_courses = []
    removed_comments = []
