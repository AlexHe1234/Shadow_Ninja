    ...
    def __init__(
        ...
        for pf in use:
            ...
            if pf == 'unsplash':
                ...
                if pf_obj.available:
                    self.platforms.append(pf_obj)
                    print(f'Added platform {pf_obj.pf}')
        
        print('Engine is ready')
        
    def run(self, search_word: str, num_result: int):
        # TODO: mp + proc results
        for pf in self.platforms:
            ret = pf.run(search_word, num_result)
            
            with app.app_context():
                for url in ret:
                
                    # add to db
                    row = Url(platform=pf.pf, url=url)
                    db.session.add(row)
                    
                db.session.commit()
                print(f'Added {len(ret)} links to database')
